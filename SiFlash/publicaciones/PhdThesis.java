...
public class PhdThesis extends Publication 
{
	 ...
	 */
	public PhdThesis(LinkedList<Campo> campos)
	{
		Campo campo;
	...
	public static Vector<PhdThesis> generaPub(Vector<Object[]> v) throws UnimplementedException {
		...
		for (int i=0; i< v.size();){
			...
			PhdThesis phd1 = new PhdThesis(idDoc,referencia,title,Integer.toString(year),month,URL,abstracts,note,claves,user,proyectos,autores,address,school,type);
			vector.add(phd1);
			...
			if (i>= v.size()) cambio_pub = true;
				...
				array = v.get(i);		
				idDoc = ((Long) array[0]).intValue();
				if (idDoc != phd1.getIdDoc()) cambio_pub = true;
			...
			while (!cambio_pub){
				...
				if (proyecto != null) phd1.addProyect(proyecto);
				if (clave != null) phd1.addKey(clave);
