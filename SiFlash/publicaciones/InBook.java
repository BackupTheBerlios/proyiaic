...
public class InBook extends Publication 
{
	...
	public static Vector<InBook> generaPub(Vector<Object[]> v) throws UnimplementedException {
		Vector <InBook> vector = new Vector <InBook>();
		if (v == null) return vector;
		for (int i=0; i< v.size();){
			Object[] array = v.get(i);
			int idDoc,year,number,id_aut;
			String title,volume,series,pages,address;
			...
			InBook inb1 = new InBook(idDoc,referencia,title,Integer.toString(year),month,URL,abstracts,note,claves,user,proyectos,autores,edition,type,editores,volume,Integer.toString(number),series,pages,address,chapter,publisher);
			vector.add(inb1);
	...
	public InBook(int idDoc, String referencia, String title,
			...
			Vector<String> proyectos,LinkedList<AutorEditor> author, String edition,
			String type, LinkedList<AutorEditor> editor, String volume,
			String number, String series, String pages, String address,
			String chapter, String publisher) throws UnimplementedException {
		this.author = author;
